...
private:
  ...
  bool use_srr_;             // Use SRR grid for O(1) lookup (default: true)
  bool use_outlier_removal_; // Use outlier removal in training (default: true)
...
public:
  ...
  // Ablation setters (call before train)
  ...
  // Public method for external benchmarking (pure inference, no I/O)
  int classify_single(double x, double y);

  // Classify without SRR hint (for ablation study)
  int classify_single_no_srr(double x, double y);

  // Nearest vertex only (for ablation study - no boundary interpolation)
  int classify_nearest_vertex(double x, double y);

  // Public methods for dynamic update benchmarking
