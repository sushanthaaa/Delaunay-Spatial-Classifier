...
void DelaunayClassifier::build_srr_grid() {
    if (dt.number_of_vertices() == 0) return;
    ...
    srr.min_x = min_x - 0.1; srr.max_x = max_x + 0.1;
    srr.min_y = min_y - 0.1; srr.max_y = max_y + 0.1;
    ...
    for (auto f = dt.finite_faces_begin(); f != dt.finite_faces_end(); ++f) {
        Point centroid = CGAL::centroid(f->vertex(0)->point(), f->vertex(1)->point(), f->vertex(2)->point());
        
        int c = (centroid.x() - srr.min_x) / srr.step_x;
        int r = (centroid.y() - srr.min_y) / srr.step_y;

        if (c < 0) c = 0; if (c >= srr.cols) c = srr.cols - 1;
        if (r < 0) r = 0; if (r >= srr.rows) r = srr.rows - 1;

        int index = r * srr.cols + c;
        srr.buckets[index] = f;
    }
    
    std::cout << "SRR Grid Built: " << k << "x" << k << " buckets (O(1) Indexing Enabled)." << std::endl;
}
...
void DelaunayClassifier::predict_benchmark(const std::string& test_file, const std::string& output_file) {
    ...
    double avg_time = test_points.empty() ? 0.0 : duration_us.count() / (double)test_points.size();

    std::cout << "------------------------------------------------" << std::endl;
    std::cout << "Total Inference Time: " << duration_us.count() / 1000.0 << " ms" << std::endl;
...
void DelaunayClassifier::export_visualization(const std::string& mesh_file, const std::string& boundary_file, const std::string& points_file) {
...
void DelaunayClassifier::run_dynamic_visualization(const std::string& stream_file, const std::string& out_dir) {
    auto stream_points = load_labeled_csv(stream_file);
    Vertex_handle hint = dt.finite_vertices_begin();
    std::vector<Vertex_handle> handles;
    ...
    export_visualization(out_dir + "/dynamic_1_inserted_triangles.csv", out_dir + "/dynamic_1_inserted_boundaries.csv", out_dir + "/dynamic_1_inserted_points.csv");
    ...
    export_visualization(out_dir + "/dynamic_2_moved_triangles.csv", out_dir + "/dynamic_2_moved_boundaries.csv", out_dir + "/dynamic_2_moved_points.csv");
    ...
    export_visualization(out_dir + "/dynamic_3_deleted_triangles.csv", out_dir + "/dynamic_3_deleted_boundaries.csv", out_dir + "/dynamic_3_deleted_points.csv");
